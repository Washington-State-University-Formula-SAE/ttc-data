...
with open('C:/Users/maxwe/Downloads/FSAE/2023-2024 Car/Repo/engine_data.pkl', 'rb') as f:
...
# Below class evaluates a set of arcs with the exit and entry speeds known, and dx known
class arcEvaluator:
    def run(self, entr_spd, ext_spd, dx, a, t_len, t_rad):
                
        # Finding total length of track
        trck_len = np.sum(t_len)

        # defining n, rounds to the nearest integer
        n = int(trck_len/dx)

        # nodespace
        nds = np.linspace(0,trck_len,int(n+1))

        # Determining maximum lateral acceleration for every turn
        t_vel = np.sqrt(a*t_rad)

        # List showing radius at every node. Used to calculate maximum tangential acceleration
        nd_rad = np.zeros(int(n))

        # The loop assigns each element with a radius 
        for i in np.arange(len(t_len)):
            nd_rad[int(np.ceil(np.sum(t_len[0:i])/dx)):int(np.ceil(np.sum(t_len[0:i+1])/dx))] = t_rad[i]
        t_rad[-1] = t_rad[-2]
        ...
        for i in np.arange(len(t_len)):
            v1[int(np.ceil(np.sum(t_len[0:i])/dx)):int(np.ceil(np.sum(t_len[0:i+1])/dx))] = t_vel[i]
        v1[0] = entr_spd
        v1[-1] = v1[-2]
        ...
        for i in np.arange(len(t_len)):
            v2[int(np.ceil(np.sum(t_len[0:i])/dx)):int(np.ceil(np.sum(t_len[0:i+1])/dx))] = t_vel[i]
